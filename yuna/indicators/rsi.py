...
class Rsi(TechnicalIndicator):
    ...
    def _handle(self):
        ...
        rsi1 = \
            Sma(self._max(opt_list), self.N1, 1, handle='on') / Sma(self._abs(opt_list), self.N1, 1, handle='on') * 100
        self.ans.append(rsi1.ans)
        rsi2 = \
            Sma(self._max(opt_list), self.N2, 1, handle='on') / Sma(self._abs(opt_list), self.N2, 1, handle='on') * 100
        self.ans.append(rsi2.ans)
        rsi3 = \
            Sma(self._max(opt_list), self.N3, 1, handle='on') / Sma(self._abs(opt_list), self.N3, 1, handle='on') * 100
